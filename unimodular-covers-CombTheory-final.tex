...
Observe that the assumption $u\in((p_1, p_2) + \vecline q$ implies that one of the segments $[\tilde u,\tilde q_i]$ crosses the interior of one of the triangles $\conv(\tilde p_1,\tilde p_2,\tilde q_j)$, where $\{i,j\}=\{1,2\}$. Without loss of generality assume that $[\tilde{u},\tilde{q}_2]$ crosses $\conv(\tilde{p}_1,\tilde{p}_2,\tilde{q}_1)$, as in \Cref{fig:flip}. 
...
\end{enumerate}
\end{claim}
\Giulia{Do we ever use the claim 3(1), other than to prove 3(2)? If not, we can remove it from the statement...}
...
\caption{Illustration of the proof of \Cref{claim:g}. The functional $g$ is constant along the dashed diagonal lines}
\label{fig:claim4}
\end{figure}
...
We consider two cases, depending on whether $q_1+q_2-p_1$ lies ``to the right'' or ``to the left'' of $p_1$. That is, whether $f_p(q_1+q_2-p_1)$ is positive or negative. %For illustration use the left part of Figure~\ref{fig:claim4} in which $f_p(q_1+q_2-p_1)=0$, and in which both arguments work:
\giulia{we put the origin in $q_1$, so the functional $f_p$ is not $0$ at $p$... so we could say $q_1 + q_2-p_1$ in $V_1$ or $V_2$... Though I'm not against changing our origin to lie in the intersection of $p$ and $q$}
...
$r'=[r'_1,r'_2]$ is parallel to $[r''_1,r''_2]$ but shorter, \giulia{I would say that $r'$ needs to lie between the lines along $b$ and $t$ to explain why it's shorter} so that
\[
...
Consider the segment $s$ contained in $r_1' + \vecline p$ with endpoints $s_1=r_1'$ and $s_2$ on the line spanned by $t$. The width of $s$ with respect to $g$ is $g(s_2)-g(s_1) <1$, because $s$ is shorter than $Q \cap p$,\giulia{we still need to explain why this is true...} which is strictly contained between the consecutive parallel lattice lines through $p_1$ and $q_2$ and through $q_1$ and $q_1+q_2-p_1$ where $g$ is constant.

We now observe that $g(r'_2) < g(s_2)$, because $r'_2$ lies to the right of $s_2$ ($r$ has positive slope) and below the line spanned by $t$, and $g$ decreases moving to the right along $t$ (\Cref{claim:g}). Thus $g(r_2)-g(r_1) = g(r'_2)-g(r'_1) < g(s_2)-g(s_1) <1$. Since $g(r_2)$ and $g(r_1)$ are integers this leads to $g(r_2)\le g(r_1)$, in contradiction with \Cref{claim:g}.
...
\bibitem{BGM}
  Winfried Bruns, Joseph Gubeladze, and Mateusz Micha{\l}ek. Quantum Jumps of Normal Polytopes. \emph{Discrete Comput. Geom.}, 56(1) (2016), 181--215. \doi{10.1007/s00454-016-9773-7}
