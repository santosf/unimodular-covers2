...
Let $g$ be the primitive lattice functional constant on $[p_1, q_2]$ (and therefore constant also on $[q_1, q_1+q_2-p_1]$). By the assumption that $f_q(p_1)=-1$ the quadrilateral spanned by these two parallel segments is unimodular, so  the values of $g$ on them differ by $1$. We choose the sign of $g$ so that 
\[
...
Since the normal fan of $Q$ refines that of $P$, $Q$ must contain an edge parallel to $r$ on the right. Let $r'$ be a primitive segment along this edge, so that $r'$ is a translated copy of $r$.
...
We now have a contradiction. Claim~\ref{claim:g} says that $g(r_2) - g(r_1) >0$ and Claim~\ref{claim:g2}  that $g(r_2) - g(r_1) <1$. Since  $g(r_2) - g(r_1)$ is an integer, this is impossible.
\end{proof}
