...
\begin{claim}
%\paco{new claim. Note I changed the roles of $r'$ and $r''$, since (the new) $r''$ is only used inside the claim}
\label{claim:r}
...
We are now ready to show a contradiction. We consider two cases, depending on whether $q_1+q_2-p_1$ lies ``to the right'' or ``to the left'' of $p_1$. That is, whether $f_p(q_1+q_2-p_1)$ is positive or negative. %For illustration use the left part of Figure~\ref{fig:claim4} in which $f_p(q_1+q_2-p_1)=0$, and in which both arguments work:
...
\]
while Claims~\ref{claim:r} and~\ref{claim:width} imply that $\width_{f_q}(r')\ge \width_{f_q}(r)  \ge w\ge 2$.

\item \underline{$f_p(q_1+q_2-p_1)\le 0$}.
We observe that $g(r_2) - g(r_1)$ is positive (and integer) because $r$ has positive slope and separates $p_1$ from $q_2$. But Claim~\ref{claim:r} gives
\[
