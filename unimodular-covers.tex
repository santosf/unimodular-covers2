...
The condition that the normal fan of $Q$ is finer than that of $P$ is sometimes dubbed \emph{$P$ is a weak Minkowski summand of $Q$}, since it is equivalent to the existence of a polytope $P'$ such that $P+P' = k Q$ for some dilation constant $k>0$~\cite[??]{Grunbaum}. 
...
(P+Q) \cap \Z^d = P \cap \Z^d + Q \cap \Z^d.
\]
In particular, the following statement, proved in \Cref{sec:cayley}, is stronger than the afore-mentioned result of \cite{Fakhruddin,HNPS2008,Ogata}:
...
This also implies the following two statements, also proved in \Cref{sec:cayley}.
A  \emph{prismatoid} is a polytope whose vertices all lie in two parallel facets. 
\paco{Wait; ``width one'' is only a special case of ``prismatoid'' if we allow prismatoids to have vertices in two parallel \emph{faces}, not \emph{facets}. But \Cref{coro:prismatoid} is not so obvious in that more general case. Either prove it or remove the phrase ``special case''} \giulia{true...}
...
\giulia{We are using lemmas 2 and 3 which are for empty tetrahedra; either we add a step in this corollary saying that we can reduce to empty tetrahedra, or we keep this for empty and triangulate into empty tetrah in the proof of the theorem (which makes more sense to me)}
Let $T$ be a empty lattice tetrahedron contained in a lattice parallelepiped $P$. Then, $T$ can be covered by unimodular tetrahedra contained in $P$.
\end{corollary}
...
\begin{proof}[Proof of \Cref{thm:parallelepipeds}]\giulia{see comment above}
...
Throughout this section \giulia{I think this may be misleading, in the sense that later we specify to $d=2$ and we should maybe highlight this} let $P$ and $Q$ be two lattice polytopes in $\R^d$. We do not require them to be full-dimensional, but we assume their Minkowski sum is. Remember that the Minkowski sum $P+Q$ of $P$ and $Q$ is defined as:
\[
...
\begin{center}
  \begin{tabular}{ c | c }
    $\cayley(P,Q)$ & $P+Q$\\
    \hline
   polyhedral subdivisions & mixed subdivisions \\
    triangulations & fine mixed subdivisions \\
   unimodular simplices & unimodular prod-simplices \\
  \end{tabular}
\end{center}\giulia{not sure if the table makes it clear that these are bijections}
