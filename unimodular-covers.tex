...
\paco{define mixed IDP, Cayley sums, Minkowski sums, mixed subdivisions? Probably not}
...
\item 
\label{itm:mixedIDP}
(Related to problems 1, 3, 4, 6 in \cite{Oda1997}) Let $P, Q\subset \R^d$ be lattice polytopes with the normal fan of $Q$ refining that of $P$, and $P$ smooth. Then
\begin{align*}
\label{eq:mixedIDP}
...
This not only implies \Cref{conj:Oda}(1) in dimension three, but also the following two statements, that we prove in \Cref{sec:cayley}.
A $3$-dimensional \emph{prismatoids} is a polytopes whose vertices all lie in two parallel facets. A special case is a polytope of width $1$, whose vertices lie in two consecutive parallel lattice hyperplanes.
...
\begin{corollary}
Every integer dilation of a lattice $3$-polytope of width $1$ has a unimodular cover.
\end{corollary}


We believe that the $3$-polytopes in all these statements all have unimodular triangulations, but this remains an open question.
