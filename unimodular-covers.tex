...
claim: if we tilt on of the $f_i$s so that they are parallel, there is no room to fit $e_i$, since it intersects both. 
...
\begin{proof}
$l_2, r_2, b_1, t_1 \ge 1$ follows from the fact that each edge crosses the corresponding strip. If one of the inequalities is not strict then one of the strips contains half of the square $p_1p_2+q_1q_2$, hence it contains a lattice point in its interior.  
\end{proof}
...
\begin{proof}
The assumption implies that the intersections of $t$ and $b$ with the vertical line $\{x=1-\beta\}$ through $q_2$ are less than distance one apart (consider a vertical lattice line intersecting $q_1q_2$ and two consecutive lattice points in it, one above and one below the horizontal axis). This in turn implies part (a). For (b), if $r_1$ was $\le 0$ it would be impossible to fit a translated copy of $r$ in the right side of $Q$, because of (a).
\end{proof}
