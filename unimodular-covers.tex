...
Since these all have volume strictly smaller than $T$ \giulia{not convinced this is so obvious or true for any $l$: the volume of $\conv(T \cup q_4)$ is 3/2 times that of $T$, and if two of the three have very small volume... It is true that if we took instead of $l$ $q_4$, since the segment $p_4q_4$ intersect the triangle in the baricenter, we would have three of the same volume, but it's not true for $l$..... But I think (perhaps check in coordinates) we can choose $l$ wisely.}, this allows us to cover $T$ with lattice simplices of smaller volume recursively, until it is covered by unimodular ones.
\end{proof}
