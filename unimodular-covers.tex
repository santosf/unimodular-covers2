...
The idea to have in mind is that of the regular tetrahedron inscribed in a cube, see \Cref{fig:circumscribed_parall}. 
Modulo an affine transformation this is exactly the situation of $T$ and $C(T)$.

\begin{figure}[htb]
\includegraphics[width=.7\textwidth]{circumscribed_parall}
\caption{In red we have the tetrahedron $T$, in black the circumscribed parallelepiped $C(T)$, and in blue the corner simplex $T_4$.}
\label{fig:circumscribed_parall}
\end{figure}
\giulia{I added a picture, now we can decide if we want it in the paper or not.}
...
\begin{figure}[htb]
\includegraphics[width=.9\textwidth]{Whites}
\caption{The parallelograms $\conv(p_1p_2q_3q_4)$ and $\conv(q_2q_1p_4p_3)$. The large red points are lattice points.}
\label{fig:corner}
\end{figure}
\giulia{ditto}
...
\giulia{just changed small things}
...
\begin{figure}[htb]
\includegraphics[width=.9\textwidth]{flip}
...
We let $H_1=\{f_q(x) \leq 0\}$ and $H_2=\{f_q(x) \geq 0\}$; similarly let $V_1=\{f_p(x) \leq 0\}$ and $V_2=\{f_p(x) \geq 0\}$. \giulia{we need to make these closed, so that the case $P$ one dimensional fits into this same proof}
