...
\end{align*}
%
A lattice polytope $Q$ is called \emph{smooth} if it is simple and the edge generators at every vertex form a linear basis for the lattice; equivalently, if the projective toric variety defined by the normal fan of $Q$ is smooth. 
...
The two-dimensional case of \Cref{conj:Oda}\eqref{itm:mixedIDP} is known to hold, with three different proofs by Fakhruddin~\cite{Fakhruddin}, Ogata~\cite{Ogata} and Haase et al.~\cite{HNPS2008}. This last one actually shows that smoothness of $Q$ is not needed. In dimension three, however, the conjecture fails without the smoothness assumption, as shown by letting $P=Q$ be any non-unimodular \emph{empty tetrahedron}, by which we mean a lattice tetrahedron with no lattice points other than its vertices (see the proof of \Cref{lemma:corner} for details on them).
\paco{added def of empty tetrahedron}
...
\begin{center}
  \begin{tabular}{ c c c }
    $\cayley(P,Q)$ && $P+Q$\\
    \hline
   polyhedral subdivisions &$\leftrightarrow$& mixed subdivisions \\
    triangulations &$\leftrightarrow$& fine mixed subdivisions \\
   unimodular simplices &$\leftrightarrow$& unimodular prod-simplices \\
  \end{tabular}
...
We now turn our attention to $d=2$, in order to prove \Cref{thm:cayley}. In a triangulation of $\cayley(P,Q)\subset \R^3$ there are tetrahedra of types $(1,3)$, $(2,2)$ and $(3,1)$, where the type denotes how many vertices they have in $P$ and in $Q$. Empty tetrahedra of types $(1,3)$ or $(3,1)$ are automatically unimodular, so we only need to care with those of type $(2,2)$. These are the Cayley sums of a segment $p\subset P$ and a segment $q\subset Q$.
%
%
...
\paco{inserted this corollary to make proof clearer, and to parallel section 2}
...
Let us show how to derive \Cref{coro:prismatoid,coro:width1} from this.
Recall that \emph{prismatoids} were defined in~\cite{Santos-hirsch} as polytopes whose vertices all lie in two parallel facets. In particular, a \emph{lattice prismatoid} is any $d$-polytope $SL(\Z,d)$-equivalent to one of the form
\[
\conv(Q_1\times\{0\} \cup Q_2 \times \{k\}),
\]
where $Q_1,Q_2$ are lattice $(d-1)$-polytopes and $k\in \Z_{>0}$. This generalizes Cayley sums, which are the case $k=1$, except the definition of prismatoid requires $Q_1$ and $Q_2$ to be full-dimensional, while the Cayley sum only requires this for $Q_1+Q_2$.
...
\]
is a Cayley polytope in which both bases have the same normal fan (if $i\in\{2,\dots,k-1\}$ or one is a weak Minkowski summand of the other (if $i\in \{1,k\}$).
\end{proof}


\begin{proof}[Proof of \Cref{coro:prismatoid,coro:width1}]
In both cases the polytope under study satisfies the hypotheses of \Cref{prop:prismatoid}. For the first, smoothness of a prismatoid implies that every edge of the form $[u\times \{0\}, v\times \{k\}]$ has lattice points in all slices. For the second, if $P$ has width one then $P\cong \cayley(Q_1, Q_2)$ for some $Q_1$ and $Q_2$. Hence,
\[
