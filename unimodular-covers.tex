...
Observe that one of $l$ and $r$ can coincide with $p$, if this is on the boundary of $P$, and similarly one of $t,b$ might be $q$, if this is on the boundary of $Q$. 
\giulia{p=P ???}
...
\begin{claim}
$f_q(b_2-b_1)$ and $f_q(p_2 - p_1)$ are non-zero and have the same sign. That is, $f_q$ achieves its maximum over $b$ and over $t$ on the same halfplane $V_1$ or $V_2$.
...
\begin{proof}
Suppose by contradiction that the maximum of $f_q$ on $t$  lies in $V_1$ and that on $b$ lies in $V_2$. 
