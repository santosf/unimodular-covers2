...
\Paco{
I am a bit puzzled. After the next corollary we say that \cite[Cor.~4.2]{SantosZiegler} proves a case of this, but in fact it proves the whole of it, in a stronger form that does not need width one, and that gives a "dissection" and not a mere cover (dissection= cover by simplices with disjoint interiors). 

I propose to change it to the following, related to a recent preprint of Gubeladze:
...
Here ee call ``lattice boundary'' of a polytope the union of all the prismatoids obtained translating each facet hyperplane one lattice unit towards the interior. Put differently, if $\{a_i x \le b_i, i=1,\dots,m\}$ integer is an integral facet description of $P$, the lattice boundary is $P$ minus the interior of $P'=\{a_i x \le b_i-1, i=1,\dots,m\}$. Remark: $P'$ is related to (but not exactly the same as) the ``Fine interior'' of $P$. See eg, some papers by Batyrev on this.
