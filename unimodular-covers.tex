...
%
Oda~\cite{Oda1997} posed several questions regarding IDP polytopes which are now considered conjectures~\cite{HNPS2008,mfo2007}. 
Among them:
\begin{conjecture}
\label{conj:Oda}
\begin{enumerate}
...
In \Cref{sec:parallelepipeds} we show the following stronger statement:
...
An alternative approach to \Cref{conj:Oda}(1) is via Cayley sums, that we discuss in  \Cref{sec:cayley}. 
As we note there (\Cref{prop:mixedIDP}), if $P,Q\subset \R^d$ are two lattice polytopes one has
\[
\cayley(P,Q) \text{ is IDP} 
\Rightarrow
(P+Q) \cap \Z^d = P \cap \Z^d + Q \cap \Z^d.
\]
In particular, the following statement that we prove in \Cref{sec:cayley} is stronger than the afore-mentioned result of \cite{Fakhruddin,HNPS2008,Ogata}:
...
This not only implies \Cref{conj:Oda}(1) in dimension three, but also \Cref{conj:Oda}(2) for $3$-dimensional \emph{prismatoids}, polytopes whose vertices all lie in two parallel facets:
...
EXCEPT FOR THIS TO BE TRUE WE NEED TO ALLOW $Q$ TO DEGENERATE TO AN EDGE IN \Cref{hm:cayley}. CAN WE?
}
...
Our proof of \Cref{thm:cayley} is based on the following result, the proof of which we postpone to \Cref{sec:the_lemma}. 
The lemma uses the following conventions. If $a, b$ are points, we denote by $[a,b]$ and $(a,b)$ respectively the closed and open segments with endpoints $a,b$. For a segment $s=[a,b]$, we denote the vector $\vec s:= b-a$ and $\vecline s$ as the line spanned by $\vec s$.

\begin{lemma}
\label{lemma:cayley}
Let $P$ and $Q$ be two lattice polygons with $Q$ nef with respect to $P$. Let $p=[p_1,p_2] \subset P$ and $q=[q_1,q_2]\subset Q$ be two primitive and non-parallel lattice segments, and let $\vecline p$ and $ \vecline q$ be the lines spanned by them.  If the parallelogram $p + q$ is not unimodular, then at least one of the regions
\[
...
\paco{add picture for these strips}
...
We now turn our attention to \emph{prismatoids}, defined in~\cite{Santos-hirsch} as polytopes whose vertices all lie in two parallel facets. Since we are only interested in lattice polytopes, we here use \emph{prismatoid} meaning any $d$-polytope of the form
\[
\conv(Q_1\times\{0\} \cup Q_2 \times \{k\}),
\]
with $k\in \Z_{>0}$ and $Q_1,Q_2$ being lattice $(d-1)$-polytopes. This generalizes Cayley sums, which are the case $k=1$.

\begin{proposition}
\label{prop:prismatoid}
Let $Q_1$ and $Q_2$ be two lattice polygons and consider the prismatoid 
\[
...
\Cref{coro:prismatoid} follows from \Cref{prop:prismatoid} since smoothness of a prismatoid implies $P\cap(\R^2\times\{i\})$ to be a lattice polytope for every $i$.
...
By a change of coordinates and lattice, we can assume $p=[p_1,p_2]$ and $q= [q_1,q_2]$ to be a vertical and horizontal segment . Since translating the polytopes by lattice vectors will not result in any loss of generality, we can further assume that $p_1$ is on the next lattice line parallel to the line spanned by $q$. That is, if $f_q$ is the primitive lattice functional which is zero on $q$, then $f_q(p_1)=-1$. Equivalently, the triangle $\conv(q_1, q_2, p_1)$ is unimodular. 

Let $f_p$ be the primitive lattice functional constant on $p$. Let $w=\area_\Lambda(p+q) \geq 2$, where $\area_\Lambda$ denotes the area normalized to a fundamental domain of $\Lambda$. Then 
\[
w=\width_{f_q}(p + \vecline q )=\width_{f_q}(p)=\width_{f_p}(q)=\width_{f_p}(q +\vecline p ).
\] 

Observe that $p_1$ is the unique lattice point in the strip $q +\vecline p$ on the line $f_q(x)=-1$. Indeed, since $q$ is primitive, the only way that in the strip there could be two lattice points on $f_q(x)=-1$ is if they were on the boundary of the strip, which would however imply that $p+q$ is a unimodular paralellogram, which is a contradiction. Similarly, the unique lattice point in the strip on the line $f_q(x)=1$ is then $q_1+q_2 -p_1$.

Suppose by contradiction that there is no lattice point as described in the lemma. In particular, no vertex of $P$ can be in the strip $p + \vecline q$. The same holds for $Q$ and its corresponding strip. Thus $P$ has two primitive segments in the boundary which each have one vertex on each side of the strip $p + \vecline q$; we will call these  $\ell$ and $r$ (with $\ell$ to the left and $r$ to the right). The same holds for $Q$ and the strip $q + \vecline p$, and we call the segments  $b, t$ (with $b$ below and $t$ above).  Observe that one of $l$ and $r$ can coincide with $p$, if this is on the boundary of $P$, and similarly one of $t,b$ might be $q$, if this is on the boundary of $Q$. 
\giulia{p=P ???}
...
\begin{proof}
The inequality $\geq w$ follows in each case from the fact that each segment crosses the corresponding strip, the width of which (w.r.t the appropriate functional) is $w$, as observed above. 
If one of the inequalities, say the one for $\ell$, is not strict, then $\ell$ has one endpoint on each of the boundary lines of $(p + \vecline q)$. Unless $\ell = p$, one of the endpoints of $\ell = [l_1, l_2]$ is not an endpoint of $p$, say $l_1 \neq p_1$. Thus the triangle $T=\conv(p_2, p_1, l_1)$ is contained in $P$ and its edge $[p_1, l_1]$ is an integer dilation of $q$. Since $\width_{f_q}(T) =w \geq 2$, $T$ must contain a lattice point in the interior of the strip.
\end{proof}
...
%\end{proof}
Let $H^-$ and $H^+$ be the two halfplanes defined by the affine span of $p$, with $H^+$ to the right.
\begin{claim}
$f_q$ achieves its maximum over $b$ and over $t$ on the same side ($H^+$ or $H^-$). 
\end{claim}\giulia{do we need this claim?}
...
\begin{proof}
 Suppose by contradiction that the maximum of $f_q$ on $t$  lies in $H^-$ and that on $b$ lies in $H^+$. 

Then $Q \cap H^ +$ is contained in the open strip $\{-1<f_q(x)<w-1\}$, of width $w$. This cannot contain a translated copy of $r$, since $\width_{f_q}(r) \geq w$ .
\end{proof}
...
We assume w.l.o.g. that the maximum on both $t$ and $b$ is achieved in $H^+$ (do we need it??).

\begin{claim}
\label{positive slopes}
Assume w.l.o.g.~that $b$ and $t$ either are parallel or their affine spans cross in $H^+$. Then, 
\begin{enumerate}
\item The intersection of $Q$ with any line parallel to $p$ in $H^+$ has width w.r.t. $f_q$ strictly smaller than $w$.
\item $f_p$ and $f_q$ increase in the same direction along $r$.
\end{enumerate}
\end{claim}
...
For part (b), let $r = conv(u, v)$, with $\vec r=v-u$. If $r$ had negative slope, it would be impossible to fit a translated copy of $r$ in the right side of $Q$: $r$ would lie inside the region to the left of $u$ delimited by $t$, the horizontal line containing $u$ and the vertical line through $u$. We can observe that this triangular region has width w.r.t. $f_q$ less than $i$ by combining part (a), which implies that the vertical segment through $u$ between $t$ and $u$ has width w.r.t $f_q$ less than $i$, with the fact that $t$ has positive slope. 
\end{proof}
...
Let $g$ be the functional constant on $[p_1, q_2]$. Then
\begin{claim}
The slopes of $t=[t_1, t_2]$ and $b=[b_1, b_2]$ are strictly smaller than the slope of  and that of $r=[r_1, r_2]$ (and $l=[l_1, l_2]$) is strictly greater than it.
\end{claim}\giulia{let's put some more notation in the beginning: add H+, H-, V+, V-, and say which endpoints of l, r, etc are in which halfspace.}
...
\bibitem{Ogata}
Shoetsu Ogata.
\newblock {Multiplication maps of complete linear systems on projective toric
  surfaces.}
