...
Work of F. Santos is also supported by grants  MTM2017-83750-P/AEI/10.13039/501100011033 and PID2019-106188GB-I00/AEI/10.13039/501100011033 of the Spanish State Research Agency}
...
\end{abstract}
\paco{added ``smooth'' to c.s. polytopes in abstract}
...
%\paco{did not find this in the google books version of \cite{Grunbaum}; found it in Postnikov-Reiner-Williams, Tsuchiya, a survey by Rekha Thomas, and other places. Perhaps we can leave it wothout reference} 
...
\Paco{
I am a bit puzzled. After the next corollary we say that \cite[Cor.~4.2]{SantosZiegler} proves a case of this, but in fact it proves the whole of it, in a stronger form: it does not need width one, and it gives a "dissection" and not a mere cover (dissection= cover by simplices with disjoint interiors). 
I propose to delete it:
%
%\noindent{\bf Corollary}
%%\label{coro:gorenstein}
%Let $P$ be a lattice $3$-polytope with the following property: for every facet $F$ of $P$ and every edge $E$ of $P$ with (exactly) an end-point in $F$, there is a lattice point $v\in E$ at distance $1$ from $P$. Then, there is a unimodular cover of its ``lattice boundary''. 
%%\end{corollary}
%
%Here we call ``lattice boundary'' of a polytope the union of all the prismatoids obtained translating each facet hyperplane one lattice unit towards the interior. Put differently, if $\{a_i x \le b_i, i=1,\dots,m\}$ integer is an integral facet description of $P$, the lattice boundary is $P$ minus the interior of $P'=\{a_i x \le b_i-1, i=1,\dots,m\}$. Remark: $P'$ is related to (but not exactly the same as) the ``Fine interior'' of $P$. See eg, some papers by Batyrev on this.
%
%This corollary is related (but neither implies nor is implied by) Theorem (a) in Gubeladze's recent arXiv:2012.11846
}
{\color{red}
\begin{corollary}
%\label{coro:width1}
...
\paco{added ref to Gubeladze}
Let us also mention the following recent results of Gubeladze~\cite{Gubeladze}.
In the second one, a polytope $P$ is called \emph{very ample} if the vertex cone $\cone(P-v)$ is contained in the translation $P-v$, for every vertex $v$ of $P$.

\begin{theorem}[Gubeladze~\cite{Gubeladze}]
Let $P$ be a lattice $3$-polytope. 
\begin{enumerate}
\item If $P=E\cap \Z^3$ for some (solid) ellipsoid $E$, then $P$ has a unimodular cover.
\item If $P$ is \emph{very ample} then the boundary of $P$ has a unimodular cover. That is, there are unimodular simplices contained in $P$ whose union contains $\partial P$.
\end{enumerate}
\end{theorem}

\paco{removed closing sentence}
%\medskip
...
%\begin{figure}[htb]
%\includegraphics[scale=.28]{Whites}
%\caption{The parallelograms $\conv(p_1p_2q_3q_4)$ and $\conv(q_2q_1p_4p_3)$. The large red dots are lattice points.}
%\label{fig:corner}
%\end{figure}
...
\end{question}
\Paco{
The following is a counter-example: Let $P$ be the Cayley embedding of a long horizontal rectangle and a long vertical rectangle. That is,
...
A weaker version of the question is: 

Question 2.5Õ. If $T$ is an empty tetrahedron contained in a smooth 3-polytope $P$, can one guarantee that there is a lattice point of $P$ in the circumscribed parallelepiped of $T$ (apart of the vertices of $T$)?

A positive answer would still imply that $P$ can be unimodularly covered.
}
...
\paco{removed references to Coro 1.8}
...
%In \Cref{coro:width1}, since $P$ has width one, $P\cong \cayley(Q_1, Q_2)$ for some $Q_1$ and $Q_2$. 
Hence,
\[
...
%\giulia{we need to make these closed, so that the case $P$ one dimensional fits into this same proof}\paco{ok}
