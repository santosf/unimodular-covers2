...
\paco{define mixed IDP, Cayley sums, Minkowski sums, mixed subdivisions in the intro? Probably not}
...
Another case of Oda's questions that has been solved is the two-dimensional case of \Cref{conj:Oda}\eqref{itm:mixedIDP}, with three different proofs by
...
An alternative approach to \Cref{conj:Oda}\eqref{itm:mixedIDP} is via Cayley sums, that we discuss in  \Cref{sec:cayley}. 
\paco{added def}
...
\]
\paco{new sentence. In fact, this implies that Cayley sums are the same as polytopes of width 1}
We normally require $\cayley(P,Q)$ to be full-dimensional (otherwise we can remove coordinates) but this does not need $Q_1$ and $Q_2$ to be full-dimensional. It only requires the linear subspaces parallel to them to span $\R^d$.
...
This also implies the following two statements, that we prove in \Cref{sec:cayley}.
A  \emph{prismatoid} is a polytope whose vertices all lie in two parallel facets. 
\paco{Wait; ``width one'' is only a special case of ``prismatoid'' if we allow prismatoids to have vertices in two parallel \emph{faces}, not \emph{facets}. But \Cref{coro:prismatoid} is not so obvious in that more general case. Either prove it or remove the phrase ``special case''}
A special case is a polytope of width $1$, whose vertices lie in two \emph{consecutive} parallel lattice hyperplanes. Observe that this is equivalent to being $SL(\Z,d)$-equivalent to a Cayley sum.
...
The idea to have in mind is that of the regular tetrahedron inscribed in a cube. Modulo an affine transformation this is exactly the situation of $T$ and $C(T)$.
...
Now, the translation of vector $\left(\frac{a-1}2,\frac{b}2,1\right)$ sends $\conv(p_1p_2q_3q_4)$ to $\conv(q_2q_1p_4p_3)$, so that the translated points $u + \left(\frac{a-1}2,\frac{b}2,1\right)$ and $u + \left(\frac{a-1}2,\frac{b}2,1\right)$ lie in $\conv(q_2q_1p_4p_3)$ and are centrally symmetrically placed in it. thus, one of them belongs to $\conv(q_1p_4p_3)$ and the other to $\conv(q_2p_4p_3)$.
...
\paco{changed this}
The parallelepiped $P$ is the intersection of three bands\paco{bands?}, where each band is a region defined as $f^{-1}([\alpha,\beta])$ for some functional $f\in (\R^3)^*$ and closed interval $[\alpha,\beta]\subset \R$. We claim that a band containing $T$ must contain at least three of the $q_i$s, which implies that the intersection $P$ of three of them contains at least one of the $q_i$s and hence $P$ contains the corresponding $T_i$.

To prove the claim suppose that $q_1\not\in B:= f^{-1}([\alpha,\beta])$ for a certain band $B$. 
...
\]
By the translation sending $q_1,q_2,p_3,p_4$ to $p_2,p_1,q_4,q_3$ (in this order) we then have
\[
...
\paco{added this intermediate result}
Let $T$ be a lattice tetrahedron contained in a lattice parallelepiped $P$. Then, $T$ can be covered by unimodular tetrahedra contained in $P$.
\end{corollary}

\begin{proof}
We proceed by induction on the (normalized) volume of $T$, which is a positive integer. If this volume equals $1$ then $T$ is unimodular and there is nothing to prove, so we assume $T$ is not unimodular.
...
\paco{removed assumption the $u$ lies in a face. I think we do not need it}
...
Observe that  $T'_4$ and/or one of the $S_i$ can degenerate to a triangle, if $u$ lies in $\conv(p_1,p_2,p_3)$, 
...
Arbitrarily triangulate the parallelepiped into lattice tetrahedra and apply \Cref{coro:coverpara} to these tetrahedra.
%Let $P \subseteq \R^3$ be a lattice parallelepiped. It can be triangulated into empty lattice tetrahedra, simply by taking a triangulation which uses all lattice points as vertices.  If all the tetrahedra in the triangulation are unimodular we are done. Otherwise, suppose $T$ is an empty tetrahedron in the triangulation which is not unimodular. \Cref{lemma:3<4} guarantees that one of the corner tetrahedra $T_i$ of the parallelepiped $C(T)$ circumscribed to $T$ is contained in $P$. Without loss of generality, suppose $T_4 = \conv(p_1, p_2, p_3,q_4)$ is in $P$. By \Cref{lemma:corner}, we know that $T_4$ contains a lattice point, which we denote by $u$. Observe that from the proof of \Cref{lemma:corner} we know more: $u$ lies in the face $\conv(p_1, p_2, q_4)$.\giulia{we need this for the volume computation below. perhaps it should be made explicit in the statemente of the lemma...}
%
%
...
contained in $P$''. If this holds then $P$ has a unimodular cover, since then the proofs of \Cref{coro:coverpara} and \Cref{thm:parallelepipeds} work for $P$.
...
Indeed, in both polytopes the only lattice points are the six vertices and the origin. The point $(1,1,1)$ lies in the second dilation but is not the sum of two lattice points in the polytope. Hence, they are not IDP, whcih implies they do not admit unimodular covers.
\giulia{is it necessary to be more explicit, for example show that these are the only lattice points?}
\paco{I think it is ok as is. It is only an example}
\end{example}
...
We now turn our attention to \emph{prismatoids}, defined in~\cite{Santos-hirsch} as polytopes whose vertices all lie in two parallel facets. In particular, a \emph{lattice prismatoid} is any $d$-polytope $SL(\Z,d)$-equivalent to one of the form
\[
\conv(Q_1\times\{0\} \cup Q_2 \times \{k\}),
\]
where $Q_1,Q_2$ are lattice $(d-1)$-polytopes and $k\in \Z_{>0}$. This generalizes Cayley sums, which are the case $k=1$.
...
This statement implies \Cref{coro:prismatoid} and \Cref{coro:width1}. 
For the former, smoothness of a prismatoid implies $P\cap(\R^2\times\{1\})$ to be a lattice polytope. \paco{details?}
For the latter, if $P$ has width one then $P\cong \cayley(Q_1, Q_2)$ and, for every $k\in \Z_{>0}$, we have
\[
...
Observe that in the strip strip $q +\vecline p$, there is a unique lattice point on the line $f_q(x)=1$;  indeed, since $q$ is primitive, the only way that in the strip there could be two lattice points on $f_q(x)=1$ is if they were on the boundary of the strip, which would however imply that $p+q$ is a unimodular paralellogram, against our assumptions.
...
\] \giulia{we want a picture for the setup}
