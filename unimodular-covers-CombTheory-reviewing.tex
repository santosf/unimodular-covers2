...
\giulia{unless b or t are equal to q}
...
\begin{claim}
\label{claim:r}
Assume without loss of generality that $b$ and $t$ either are parallel or their affine spans cross in $V_2$ (if they cross in $V_1$, the same claim can be reworded for the segment $l$). Then, 
\begin{enumerate}
...
\]
\giulia{changed the +1 to a -1, so that the following claim is true}
